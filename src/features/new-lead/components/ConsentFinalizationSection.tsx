...
export function ConsentFinalizationSection() {
  ...
  const handleToggleField = (fieldId: number) => {
    if (consentType === 'broad') return; // Cannot edit under broad consent
  ...
  const handleSubmit = async () => {
    setLocalError(null);
    const activeFields = consentType === 'broad' ? allowedFieldsList.map(f => f.id) : selectedFieldIds;
  ...
  const isFieldDisabled = consentType === 'broad';
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setConsentType('broad')}
                  className={`flex flex-col items-start p-3 rounded-lg border text-left transition-all ${consentType === 'broad'
                    ? 'border-[#16A34A] bg-[#F0FDFA] ring-1 ring-[#16A34A]'
                    : 'border-[#E5E7EB] bg-white hover:bg-gray-50'
                    }`}
                >
                  <span className="font-semibold text-[13px] sm:text-[14px] text-[#111827]">Broad Consent</span>
                  <span className="text-[11px] sm:text-[12px] text-[#6B7280] mt-1 leading-4">
