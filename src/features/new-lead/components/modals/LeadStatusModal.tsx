      ...
      >
        ...
        <div className="flex flex-row justify-between items-center px-6 py-5 border-b border-[#F1F3F4] h-[93px]">
          <div className="flex flex-col gap-1">
            <h2 className="font-inter font-bold text-[20px] leading-7 text-[#232F34] m-0">
              {isFinalize ? 'Finalize Lead Processing' : 'Update Lead Status'}
            </h2>
            <p className="font-inter font-normal text-[14px] leading-5 text-[#C1C7D0] m-0">
              Lead ID: {leadId} {isFinalize && `(Current: ${currentStatus.toUpperCase()})`}
            </p>
          </div>
          <button
            onClick={onClose}
            className="flex items-center justify-center w-8 h-8 rounded-full bg-[#F1F3F4] hover:bg-[#E2E8F0] transition-colors"
          >
        ...
        <div className="flex flex-col px-6 py-6 gap-8 overflow-y-auto">
          ...
          <div className="flex flex-col gap-3">
            <div>
              <h3 className="font-inter font-semibold text-[14px] leading-5 text-[#232F34] mb-1">
                {isFinalize ? 'Select Final Outcome *' : 'Select Outcome *'}
              </h3>
              <p className="font-inter font-normal text-[12px] leading-4 text-[#C1C7D0]">
                {isFinalize 
            ...
            <div className="flex flex-row gap-4">
              ...
              >
                <div className="flex flex-row justify-between items-start w-full mb-3">
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full ${
                    outcome === 'Rejected' ? 'bg-[#FF7676]' : 'bg-[#F1F3F4]'
                  }`}>
                    <Ban size={18} className={outcome === 'Rejected' ? 'text-white' : 'text-[#9CA3AF]'} />
                  </div>
                  {outcome === 'Rejected' ? (
                    <CircleDot size={20} className="text-[#EF4444]" />
                  ) : (
                    <Circle size={20} className="text-[#E5E7EB]" />
                  )}
                </div>
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="modal-notes" className="font-inter font-semibold text-[14px] leading-5 text-[#232F34]">
            ...
            <div className="flex flex-col border border-[#E5E7EB] rounded-lg bg-white overflow-hidden focus-within:border-[#16A34A] focus-within:ring-1 focus-within:ring-[#16A34A] transition-all">
              <textarea
                ...
                className="w-full h-[104px] p-3 text-[14px] leading-5 text-[#232F34] placeholder:text-[#D1DACE] resize-none outline-none"
              />
            </div>
            <div className="flex flex-row justify-between items-center w-full mt-1">
              <span className="font-inter font-normal text-[11px] leading-4 text-[#9CA3AF]">
                {isFinalize ? 'This note will be added to the lead\'s final audit log.' : 'This note will be added to the lead\'s audit history.'}
              </span>
              <span className="font-inter font-normal text-[11px] leading-4 text-[#D1DACE]">
                {notes.length}/500
              </span>
            </div>
          </div>
          ...
          {isFinalize && (
            <div className="flex flex-row items-start p-4 bg-[#F6F8FA] border-l-4 border-[#F59E0B] rounded-r-lg gap-3">
              <AlertTriangle size={16} className="text-[#F59E0B] shrink-0 mt-[2px]" />
              <div className="flex flex-col gap-1">
        ...
        {/* Modal Footer */}
        <div className="flex flex-row justify-end items-center px-6 py-4 bg-[#F6F8FA] border-t border-[#F1F3F4] gap-3">
          <button
            onClick={onClose}
            className="px-5 py-[10px] bg-white border border-[#E5E7EB] rounded-lg font-inter font-semibold text-[14px] leading-5 text-[#3A474E] hover:bg-slate-50 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            disabled={!outcome}
            className={`flex flex-row items-center justify-center gap-2 px-5 py-[10px] rounded-lg font-inter font-semibold text-[14px] leading-5 text-white shadow-[0px_1px_2px_rgba(0,0,0,0.05)] transition-colors ${
              outcome ? 'bg-[#16A34A] hover:bg-[#15803D] cursor-pointer' : 'bg-[#D1DACE] cursor-not-allowed'
            }`}
          >
            {isFinalize && <Lock size={12} className="text-white" />}
